								...
								<div class="table-responsive">
									<table class="table">
										...
										<tbody>
											...
											<c:forEach varStatus="loop" var="p" items="${departList}">
												<tr>
													...
													<c:url var="deleteLink" value="/delete-department">
														<c:param name="departsId" value="${p.departsId}" />
													</c:url>

													<td>${loop.index+1}</td>
													<td><a style="font-size: 14px; color: red"
														href="${deleteLink}"
														onclick="if (!(confirm('Bạn muốn xóa phòng ban này?'))) return false">Xóa</a></td>
													<td>${p.departsId}</td>
													<td>${p.departsName}</td>
													<td>${p.rewardPoint}</td>
